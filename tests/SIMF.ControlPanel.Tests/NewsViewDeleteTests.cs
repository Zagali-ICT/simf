// D-353 rollout — behaviour tests for NewsViewDelete (merged View/Delete form).
// View shows details only; Delete mode gates the soft-delete behind SimfConfirm
// (no DELETE until confirmed).
using Bunit;
using SIMF.Common;
...
public sealed class NewsViewDeleteTests : CpComponentTestBase
{
    ...
    {
        var cut = RenderComponent<NewsViewDelete>(p => p
            .Add(x => x.IsDelete, false)
            .Add(x => x.Initial, Detail()));

        Assert.Contains("Forum opens", cut.Markup);
    ...
    public void Delete_mode_gates_the_call_behind_confirmation()
    {
        ...
        Assert.Equal($"/account/api/admin/news/{row.Id}", (string)del.Arguments[0]!);
    }

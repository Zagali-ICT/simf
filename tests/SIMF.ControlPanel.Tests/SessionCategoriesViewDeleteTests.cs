// D-353 rollout â€” behaviour tests for SessionCategoriesViewDelete (merged
...
public sealed class SessionCategoriesViewDeleteTests : CpComponentTestBase
{
    ...
    {
        var cut = RenderComponent<SessionCategoriesViewDelete>(p => p
            .Add(x => x.IsDelete, false)
            .Add(x => x.Initial, Detail()));

        Assert.Contains("Keynotes", cut.Markup);
    ...
    public void Delete_mode_gates_the_call_behind_confirmation()
    {
        ...
        Assert.Equal($"/account/api/admin/session-categories/{row.Id}", (string)del.Arguments[0]!);
    }

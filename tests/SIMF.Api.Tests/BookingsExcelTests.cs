...
/// <summary>
/// Integration tests for the D-356 Bookings grid export. The booking approval
/// queue is export-only (bookings are created by visitors in the app and
/// approved/rejected from the CP), so this covers the export round-trip + the
...
public sealed class BookingsExcelTests : IClassFixture<SimfApiFactory>
{
    ...
    public BookingsExcelTests(SimfApiFactory factory)
    {
        _factory = factory;
    ...
    {
        var email = $"booking-xlsx-admin-{Guid.NewGuid():N}@simf.test";
        using (var scope = _factory.Services.CreateScope())
        {
            ...
            var user = new SimfUser
            {
                ...
                Email = email,
                EmailConfirmed = true,
                DisplayName = "Test Booking Excel Admin",

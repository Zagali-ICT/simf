...
/// the reference resource (Interests): export round-trip, a positive import,
...
public sealed class InterestExcelTests : IClassFixture<SimfApiFactory>
{
    ...
    public InterestExcelTests(SimfApiFactory factory)
    {
        _factory = factory;
    ...
    public async Task Export_returns_an_xlsx_workbook()
    {
        ...
        await CreateInterestAsync(adminToken, $"Export A {Guid.NewGuid():N}");
        await CreateInterestAsync(adminToken, $"Export B {Guid.NewGuid():N}");

        var response = await PostAuthAsync(
            "/api/v1/admin/interests/export",
    ...
    public async Task Import_creates_each_row_and_reports_the_outcome()
    {
        ...
        var workbook = BuildInterestsWorkbook("Interests",
            (nameOne, "مستورد ١", 3),
            (nameTwo, "مستورد ٢", 4));

        var response = await PostFileAuthAsync(
            "/api/v1/admin/interests/import", workbook, adminToken);
        ...
        var list = await PostAuthAsync(
            "/api/v1/admin/interests/list", new GridQuery { Top = 200 }, adminToken);
    ...
    public async Task Import_reports_a_per_row_error_for_a_duplicate_without_aborting()
    {
        ...
        await CreateInterestAsync(adminToken, existing);
    ...
    public async Task Import_rejects_a_file_that_is_not_a_workbook()
    {
        ...
        var response = await PostFileAuthAsync(
            "/api/v1/admin/interests/import", notXlsx, adminToken);
    ...
    {
        var adminToken = await CreateAdministratorAndSignInAsync();
        var wrongSheet = BuildInterestsWorkbook("NotInterests",
            ($"X {Guid.NewGuid():N}", "س", 1));

        var response = await PostFileAuthAsync(
            "/api/v1/admin/interests/import", wrongSheet, adminToken);
    ...
    private static byte[] BuildInterestsWorkbook(
    ...
    private async Task CreateInterestAsync(string token, string name)
    {
        var response = await PostAuthAsync(
            "/api/v1/admin/interests",
            new AdminCreateInterestRequest { Name = name, NameArabic = "اهتمام", DisplayOrder = 0 },
            token);
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
    }
    ...
    {
        var email = $"interest-xlsx-admin-{Guid.NewGuid():N}@simf.test";
        using (var scope = _factory.Services.CreateScope())
        {
            ...
            var user = new SimfUser
            {
                ...
                Email = email,
                EmailConfirmed = true,
                DisplayName = "Test Interest Excel Admin",

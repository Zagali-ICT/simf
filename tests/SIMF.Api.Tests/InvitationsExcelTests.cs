...
/// <summary>
/// Integration tests for the D-356 Invitations grid export. Invitations are
/// created/edited from the CP forms, so the resource is export-only — this covers
/// the export round-trip (a valid XLSX workbook) + the Export permission gate.
...
public sealed class InvitationsExcelTests : IClassFixture<SimfApiFactory>
{
    ...
    public InvitationsExcelTests(SimfApiFactory factory)
    {
        _factory = factory;
    ...
    public async Task Export_returns_an_xlsx_workbook()
    {
        ...
        var response = await PostAuthAsync(
            "/api/v1/admin/invitations/export",
    ...
    {
        var email = $"inv-xlsx-admin-{Guid.NewGuid():N}@simf.test";
        using (var scope = _factory.Services.CreateScope())
        {
            ...
            var user = new SimfUser
            {
                ...
                Email = email,
                EmailConfirmed = true,
                DisplayName = "Test Invitations Excel Admin",

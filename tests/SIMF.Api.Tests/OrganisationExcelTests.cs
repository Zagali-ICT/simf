...
/// <summary>
/// Integration tests for the D-356 Organisations grid export. Organisations is
/// export-only (it keeps its bespoke government-Excel bulk import), so this
...
public sealed class OrganisationExcelTests : IClassFixture<SimfApiFactory>
{
    ...
    public OrganisationExcelTests(SimfApiFactory factory)
    {
        _factory = factory;
    ...
    public async Task Export_returns_an_xlsx_workbook()
    {
        ...
        var response = await PostAuthAsync(
            "/api/v1/admin/organisations/export",
    ...
    {
        var email = $"org-xlsx-admin-{Guid.NewGuid():N}@simf.test";
        using (var scope = _factory.Services.CreateScope())
        {
            ...
            var user = new SimfUser
            {
                ...
                Email = email,
                EmailConfirmed = true,
                DisplayName = "Test Org Excel Admin",

...
/// <summary>
/// Integration tests for the D-356 Booths grid Excel engine: the export
/// round-trip (ZIP magic), the Export permission gate, and an insert-only import
/// round-trip keyed on the booth code. Test data carries Guid-unique codes so it
...
public sealed class BoothsExcelTests : IClassFixture<SimfApiFactory>
{
    ...
    public BoothsExcelTests(SimfApiFactory factory)
    {
        _factory = factory;
    ...
    public async Task Export_returns_an_xlsx_workbook()
    {
        ...
        await CreateBoothAsync(adminToken, UniqueCode());
        await CreateBoothAsync(adminToken, UniqueCode());

        var response = await PostAuthAsync(
            "/api/v1/admin/booths/export",
    ...
    public async Task Import_creates_each_row_and_reports_the_outcome()
    {
        ...
        var workbook = BuildBoothsWorkbook("Booths",
            (codeOne, "Booth One", "جناح ١", "Defence"),
            (codeTwo, "Booth Two", "جناح ٢", "Maritime"));

        var response = await PostFileAuthAsync(
            "/api/v1/admin/booths/import", workbook, adminToken);
        ...
        var list = await PostAuthAsync(
            "/api/v1/admin/booths/list", new GridQuery { Top = 200 }, adminToken);
        var page = (await list.Content
            .ReadFromJsonAsync<ApiResult<GridPage<AdminBoothSummary>>>())!.Data!;
    ...
    // A unique booth code within the 2–16 char / uppercase rule (the create
    // de-dup key) so the test is robust on the non-reset integration DB.
    private static string UniqueCode() =>
        ("B" + Guid.NewGuid().ToString("N"))[..12].ToUpperInvariant();

    private static byte[] BuildBoothsWorkbook(
        string sheetName,
        params (string Code, string Name, string NameArabic, string Sector)[] rows)
    {
    ...
    private async Task CreateBoothAsync(string token, string code)
    {
        var response = await PostAuthAsync(
            "/api/v1/admin/booths",
    ...
    {
        var email = $"booth-xlsx-admin-{Guid.NewGuid():N}@simf.test";
        using (var scope = _factory.Services.CreateScope())
        {
            ...
            var user = new SimfUser
            {
                ...
                Email = email,
                EmailConfirmed = true,
                DisplayName = "Test Booth Excel Admin",

...
appsettings.json that defines it, not from the older operations document, which
has drifted: it still describes a single shared SIMF_ prefix (there have been
four, one per host, since 2026-08-12), still lists Storage:AvatarBase and
Storage:UserIdDocumentBase as required (both removed), and still gives
Jwt:AccessTokenMinutes as 30 (the code says 5).
"""
...
def chapter_observations():
    ...
    return {
        ...
        "blocks": [
            p("This appendix records what was noticed while every page was "
              "opened and every constraint checked against the code. Nothing "
              "here was changed: a manual describes the system, it does not "
              "quietly alter it. They are listed so that the next person does "
              "not spend the same hours discovering them again.",
              "يسجّل هذا الملحق ما لوحظ أثناء فتح كل صفحة ومقارنة كل قيد بالشيفرة. "
              "ولم يُغيَّر شيء منها: فالدليل يصف النظام ولا يعدّله بصمت. وقد أُدرجت "
              "هنا كي لا يقضي من يأتي بعدُ الساعات ذاتها في اكتشافها من جديد."),

            h2("Documents that disagree with the code",
               "وثائق تخالف الشيفرة"),
            table(
                ["What a document says", "What the code does"],
                ["ما تقوله الوثيقة", "ما تفعله الشيفرة"],
                [
                    ["Several page references describe the page guard as a role "
            ...
            h2("Wording in the product that no longer matches its behaviour",
               "صياغات في المنتج لم تعد تطابق سلوكه"),
            bullets([
                "Saving a new visitor shows a message saying an invitation has "
                "been sent. No invitation is sent on that path — the wording "
                "predates the registration wizard that replaced it.",
                "Saving a new partner account shows the administrators' message, "
                "which mentions an invitation email that this path never queues.",
                "The registration form's own comment says the server approves and "
                "issues the badge in one step. That happens only when the walk-in "
                "mode is switched on, and it ships off.",
            ], [
                "يعرض حفظ زائر جديد رسالة تقول إن دعوة أُرسلت. ولا تُرسل أي دعوة "
                "في ذلك المسار — فالصياغة سابقة لمعالج التسجيل الذي حلّ محله.",
                "ويعرض حفظ حساب شريك جديد رسالة المسؤولين، التي تذكر بريد دعوة لا "
                "يضعه هذا المسار في الطابور أصلًا.",
                "ويقول تعليق نموذج التسجيل نفسه إن الخادم يعتمد ويصدر الشارة في "
                "خطوة واحدة. وهذا لا يحدث إلا عند تفعيل وضع الحضور، وهو مُشحون "
                "معطّلًا.",
            ]),

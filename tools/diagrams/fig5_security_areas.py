"""Figure. SIMF target deployment, security areas and internet egress.

UML deployment diagram. It carries the same estate as fig4_tier_separation.py,
whose three owner decisions of 2026-08-10 it keeps unchanged: the application
zone holds the API alone and is reachable only from the presentation zone; the
mobile clients address a presentation-tier edge rather than the API itself; and
stored files live on their own server, separate from the database.

This is a SEPARATE SHEET, not a redraw of sheet 4. Sheet 4 is the deployment
figure published in SIMF-LLD-003 v1.2 and stays exactly as issued; this sheet is
what v1.3 carries in its place. The same rule was applied when sheet 4 was added
beside sheet 1 rather than drawn over it.

Four elements are owner decisions of 2026-08-20 and are what distinguish this
sheet: the two security areas that group the zones, HSA over the data zone and
SSA over everything else; a load balancer in front of the API nodes; the
internet zone; and the API's two outbound calls to it, which cross both
firewalls on their way out.
...
OUT = r"d:\SIMF\System\V1.0.0\docs\diagrams\SIMF-Fig5-Security-Areas-And-Egress"
...
s = Sheet(1940, 1682,
          "SIMF target deployment, security areas and internet egress",

"""Figure. SIMF phase one system and data flow.
...
SIMF-LLD-003 section 5; the data stores are the two databases and the file store
from section 6.

A SEPARATE SHEET, not a redraw of sheet 3. Sheet 3 is the data-flow figure
...
sheet 7 beside sheet 2.

Phase one changes three things here, all customer requirements of 2026-08-30:

  * the AI service is the SITE-hosted GPT OSS 120B model on an on-site LLM
    server in HSA, not a cloud provider;
  * the e-mail relay is on site in HSA;
  * the file store is MinIO object storage over the S3 API.

One flow is corrected with them. The sheet used to send "live stream, summary
video" to the video platform, which SIMF has never done: the media team
publishes to the channel, and SIMF sends one public video id to fetch a caption
track. That is what the flow now carries, at both levels.

The services appear HERE and not on the component sheet. Only the video
platform is external now, but none of the three is a component of this
solution, so what matters about each is the data that crosses to it, which is a
data-flow question. Every flow to one is labelled with the payload it carries
...
OUT = r"d:\SIMF\System\V1.0.0\docs\diagrams\SIMF-Fig8-Phase-One-Data-Flow"

s = Sheet(1660, 1200,
          "SIMF phase one system and data flow",
...
s.path([(1000, 215), (1250, 215), (1250, 228), (1340, 228)],
       "public video id", label_at=(1170, 242))
...
# The right column carries three stores and three service entities. It is spaced
...
s.store(1000, 615, 460, 58, "D3", "MinIO", "stored files, S3 API")

s.box(1000, 740, 460, 62, "Video platform", "entity",
      "YouTube, the only external service")
s.box(1000, 818, 460, 62, "E-mail relay", "entity",
      "on-site SMTP relay, HSA")
s.box(1000, 896, 460, 62, "AI service", "entity",
      "on-site GPT OSS 120B, HSA")
...
# ----------------------------------------------------- process to external
# What actually crosses the boundary to each service, named rather than
# implied. These labels are the whole reason the three appear on this sheet.
# The video platform receives a public video id and nothing else: SIMF neither
# uploads nor relays video, and the caption track comes back on the same call.
# The label sits on the clear run between the last store and the video
# platform: at the old anchor a short label reads as a flow into D3.
s.path([(820, 640), (985, 640), (985, 770), (1000, 770)],
       "public video id", label_at=(938, 712))
...
s.note(840, 1030, 780, [
    "Processes: the module list in SIMF-LLD-003 section 5.",
    ...
    "Payloads to the three services: owner decision, 2026-08-10.",
    "The on-site AI model, the on-site mail relay and MinIO: customer",

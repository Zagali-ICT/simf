...
`tests/SIMF.Domain.Tests/SchemaFreezeTests.cs`. Before it, nothing anywhere
pinned the freeze; it was prose, which is exactly how six lifts accumulated
without the baseline text ever being corrected.

**Corrected 2026-09-07 (D-959).** That test used to fail the build if a *second
migration* appeared in either folder. It no longer does, because a second
migration is now how a schema change is delivered — see **D-959** below. What it
still fails on is a second `InitialCreate`, a baseline id other than
`00000000000000`, two migrations sharing a class name, and a filename that has
drifted from the `[Migration("...")]` attribute. The freeze itself is unchanged:
a schema change still needs a **new, named lift**.
...
### D-925 — the migration id is PINNED (SUPERSEDED in part by D-959)

`00000000000000_InitialCreate`, on both contexts. **The pinned id survives; the
regeneration workflow does not** — see D-959. Read this section for why the id is
pinned, and D-959 for how a schema change is delivered now.
...
breakage exists only after both land.

### D-959 — a schema change is a NEW migration, never a regeneration (2026-09-07)

Owner directive, and it reverses D-925's second half: *"NO DELETION WE WORKING IN
PRODUCTION, the current db must match initial Migration, and all other new
migration must build by add new migration ... NEVER DO MANUALLY."*

**Three rules.** Nothing is dropped — not a database, not a table, not a column.
`00000000000000_InitialCreate` is the baseline the live databases are **at**, not a
moving description of the current model. Every change after it is its own
`dotnet ef migrations add`, generated by the tool.

**Why the old workflow had to go.** A regenerated migration **never reaches a
database that already has one**: `__EFMigrationsHistory` records the pinned id as
applied, `MigrateAsync` finds nothing pending, and the regenerated file describes a
schema the live database never receives. `Regenerate-Migration.ps1` said so in its
own header and asked for a hand-run `docs/migrations/2026/*_Hotfix.sql` delta
alongside every regeneration. That was followed sometimes. D-944 shipped without it
on 2026-08-28 and broke every visitor profile read in production. By 2026-09-07 the
local pair was missing `ProfileIdentityDocuments`, `BadgeBatchItems` and
`UserProfile.OrganisationOther` while EF reported the schema as current — and the
production pair carries the same pinned history row and the same `10.0.8` stamp.

**As-built.** `InitialCreate` on both contexts was reverted to the schema the live
databases actually contain (the content at `96feb6cb`, id unchanged), proven exact
by a `sqlpackage` diff that emitted **zero** statements against both live
databases. The drift then came out as `20260907080012_SyncBaseline` (App) and
`20260907080019_SyncBaseline` (Identity), generated by `dotnet ef migrations add`
and rehearsed on restored copies before either live database was touched.

**Read the generated `Up()` — always.** EF's differ pairs a dropped column with an
added column of similar type and infers a `RenameColumn` with no knowledge of
meaning. `SyncBaseline` scaffolded fourteen of them, including
`UserProfiles.PassportNumber -> MobileNumber` and
`Sessions.RecordingUploadedByUserId -> RecordingFileId`. A wrong rename does not
fail loudly; it moves data from one business field into an unrelated one, and it is
harmless on a dev database where the source column is empty while being
catastrophic on production where it is not. EF prints *"review the migration for
accuracy"* for exactly this. Correcting a scaffolded rename into
`DropColumn` + `AddColumn` is part of the tool workflow, not hand-authoring.

**Rehearse before applying to anything with data.** The design-time factories read
`SIMF_DESIGN_TIME_APP_CONNECTION` / `SIMF_DESIGN_TIME_IDENTITY_CONNECTION`, so
`BACKUP`/`RESTORE ... AS <db>_Rehearsal`, point the variable at the copy, and run
`database update` there first. A collation `AlterColumn` fails on any column inside
an index, CHECK or FK; an `AddCheckConstraint` fails against existing rows that
violate it; a NOT NULL `AddColumn` with no default fails on a non-empty table. None
of those are visible by reading.

**Production is a DIFFERENT database with a DIFFERENT delta** and must never be
assumed to match a developer machine. Establish its actual schema first —
`sqlpackage /Action:Extract /p:ExtractAllTableData=false` — and diff dacpac to
dacpac offline. Hand-run hotfixes already applied there (D-944, D-945) mean an
`AddColumn` that is correct locally throws in production.

`SchemaFreezeTests` fails the build on a second `InitialCreate`, on any other
baseline id, on two migrations sharing a class name (the D-925 merge hazard, which
outlived the rule that guarded it), and on a filename that has drifted from the
`[Migration("...")]` attribute EF actually reads.

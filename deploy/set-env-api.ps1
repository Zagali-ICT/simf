...
#   SIMF_FileStorage__EncryptionKey           : openssl rand -base64 32   (D-568 file-store KEK)
...
$vars = [ordered]@{
    ...
    # --- Centralized file store (D-568) — envelope-encryption KEK ---
    # The AesGcmEnvelopeCipher fail-fasts at startup if this is missing, so it is
    # boot-critical. Must never change once files are encrypted under it.
    "SIMF_FileStorage__EncryptionKey"              = ""  # [REQUIRED][SECRET] openssl rand -base64 32 (AES-256 KEK)
    "SIMF_FileStorage__RootPath"                   = ""  # optional (default App_Data/files)
...
$bootCritical = @(
    ...
    "SIMF_FileStorage__EncryptionKey"             # Boot-critical (D-568 file-store KEK)

...
$vars = @(
    ...
    [pscustomobject]@{ Name = "SIMF_API_ConnectionStrings__SimfIdentityDb"; Value = 'Data Source=SIMF-DB-01;Initial Catalog=SIMF_Identity;Persist Security Info=True;User ID=simf_app;Password=Qazwsx@1885;Pooling=False;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=True;Application Name="SQL Server Management Studio";Command Timeout=0'; Secret = $true; Gate = $false; Note = "SIMF_Identity - users, roles, permissions, 2FA, tokens" }
    [pscustomobject]@{ Name = "SIMF_API_ConnectionStrings__SimfAppDb"; Value = 'Data Source=SIMF-DB-01;Initial Catalog=SIMF_App;Persist Security Info=True;User ID=simf_app;Password=Qazwsx@1885;Pooling=False;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=True;Application Name="SQL Server Management Studio";Command Timeout=0'; Secret = $true; Gate = $false; Note = "SIMF_App - everything else" }
    ...
    [pscustomobject]@{ Name = "SIMF_API_Jwt__SigningKey"; Value = 'EN83LOYEvBCl+BDrjrekgZOiC5ScwlR9swg4nPYrU7JVLPhQ9N2r3ybqIMJVVAn2'; Secret = $true; Gate = $false; Note = "symmetric signing key" }
    ...
    [pscustomobject]@{ Name = "SIMF_API_FileStorage__EncryptionKey"; Value = '98nsLB1aXqxJHEM8sVgoYgUJtR05GmP+J3GWbT6ILeI='; Secret = $true; Gate = $true; Note = "KEK for the centralized file store; rotating it makes every stored file undecryptable" }
    [pscustomobject]@{ Name = "SIMF_API_Storage__UserIdDocumentEncryptionKey"; Value = 'SgG6cPsv68gcspush1jSZyqbc8YxLkqHkTt69f7cTEA='; Secret = $true; Gate = $true; Note = "encrypts the national ID / Iqama / passport / mobile columns at rest (NCA A2-10)" }
    [pscustomobject]@{ Name = "SIMF_API_Ai__PromptHash__Secret"; Value = 'rfUva2gCiIBIZDp3YS2oIK2fglUYajr+G1WTNvtnbYg='; Secret = $true; Gate = $true; Note = "HMAC secret; the dev fallback is publicly derivable and would poison the AI audit trail" }
    ...
    [pscustomobject]@{ Name = "SIMF_API_SuperAdmin__TempPassword"; Value = 'Qazwsx@1885'; Secret = $true; Gate = $false; Note = "rotate after first sign-in; no monotonic runs" }
    ...
    # as set-once.
    [pscustomobject]@{ Name = "SIMF_API_SuperAdmin__TotpSecret"; Value = 'W5OAZX2DLR3BKKV5UXZEBYEE2AY3CACK'; Secret = $true; Gate = $true; Note = "base32 TOTP seed for the admin second factor" }
    ...
    [pscustomobject]@{ Name = "SIMF_API_Seed__DemoPassword"; Value = 'Qazwsx@1885'; Secret = $true; Gate = $false; Note = "non-production only - leave empty in Production" }
    ...
    [pscustomobject]@{ Name = "SIMF_API_Email__User"; Value = 'no-reply@ammn.com.sa'; Secret = $true; Gate = $false; Note = "SMTP user - the Zoho mailbox" }
    [pscustomobject]@{ Name = "SIMF_API_Email__Password"; Value = 'uYVDMZgjc1hN'; Secret = $true; Gate = $false; Note = "Zoho APP password for that mailbox - without it no OTP, verification or reset mail is sent" }
    ...
    [pscustomobject]@{ Name = "SIMF_API_Ai__Anthropic__ApiKey"; Value = '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'; Secret = $true; Gate = $false; Note = "" }
    ...
    [pscustomobject]@{ Name = "SIMF_API_WalkInMode__BadgeKey"; Value = 'f0kx1CyR2GtbGghi5Xmvv4g9yNCfNgW7VMGPd/2WJjs='; Secret = $true; Gate = $false; Note = "base64 32-byte AES key; copy to every BadgeDesk" }

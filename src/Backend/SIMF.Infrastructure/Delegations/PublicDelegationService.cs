...
/// + UserProfile both live there, so there is no cross-DB join (D-157).</summary>
internal sealed class PublicDelegationService(SimfAppDbContext appDbContext)
    : IPublicDelegationService
{
    public async Task<AppDelegations> GetAsync(CancellationToken cancellationToken = default)
    {
        var countries = await appDbContext.Countries.AsNoTracking()

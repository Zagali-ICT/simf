...
public partial class UsersList
{
    ...
    private const string PageKey = "admins";
    ...
    private string FormTitle => _form switch
    {
        FormKind.AddEdit => _isEdit
            ? string.Format(L["Admin.Users.EditRoles.Title"], _target?.Email)
            : L["Admin.Users.Add.Title"],
        FormKind.ViewDelete => string.Format(L["Admin.Users.Details.Title"], _target?.Email),
        _ => string.Empty,
    };
    ...
    private async Task OnSavedAsync(AdminUserSummary? saved)
    {
        ...
        CloseForm();
        ShowToast("success",
            wasEdit && saved is not null
                ? string.Format(L["Admin.Users.EditRoles.Saved"], saved.Email)
                : L["Admin.Users.Add.Saved"]);
        await LoadAsync();
    }

    // Details is read-only; the form never raises OnDeleted, but the shell wires
    // it for symmetry. Closing simply reloads is unnecessary — just close.
    private void OnDeletedAsync(AdminUserSummary deleted) => CloseForm();

...
/// <summary>
/// One row of the per-session attendance grid, served (server-paged)
...
/// geofence or operator QR door-scan. <c>LiveNow</c> is the
/// count currently inside (open rows). The attendee identity itself is never
/// resolved here — <c>UserId</c> is counted as an opaque Guid, with no
...
/// <summary>
/// One attendee currently present in a session's hall (an open
/// <c>HallAttendance</c> row), for the live per-session hall view.
/// Every field is resolved from the App-DB profile, with no cross-DB Identity

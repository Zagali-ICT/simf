...
class SimfTokens {
  ...
  //
  // ASPIRATION, NOT DESCRIPTION — read this before adding one. The convention
  // above (seatStateIconSize, metaIconBox) names a token for the QUANTITY it
  // holds. Much of the table below does not: it was generated by a sweep that
  // named each entry `<screen><Axis><Size>` from where the literal sat, so
  // `contactTileWidth` was a border STROKE, `cvTabHeight` a LINE-HEIGHT
  // multiplier, `guestModeScreenHeightSm` also a line height, and
  // `identityVerificationScreenFontSize` the size of an emoji glyph. Worse,
  // the Sm/Md/Lg suffix was doing real work: on `accountRememberForgot*`,
  // `Sm` was a 1.5 stroke and `Md` a 5px gap — two different physical
  // quantities separated only by a size suffix.
  //
  // Seven of those were corrected on 2026-08-20, and two entries that merely
  // re-spelled something the table already had were deleted (a second 1.5
  // checkbox stroke, now the shared `checkboxBorderWidth`; a 6px gap that is
  // the existing `gap6`). The rest of the 339 have NOT been audited — a name
  // here is a claim about a quantity, and until someone reads the call site
  // it is only a claim.
  //
  // So: name a NEW entry for what it measures, and if you touch an old one,
  // check its call site before trusting the name.

...
///
/// A YouTube link is deliberately NOT played here (D-761): a YouTube embed is an
/// Android platform WebView, and the WebView does not honour the Flutter clip
/// around the cover-crop — it paints its full, over-sized surface over the whole
/// screen instead of the 160px band. So a YouTube URL falls back to the hero's
/// banner-image band (the website can still play the same link, because it crops
/// the iframe in CSS, not on the Flutter side). To show a moving video in the
/// app hero, set `backgroundVideoUrl` to a direct MP4/HLS link.
///
/// The frame is cover-fitted (cropped, never letterboxed) to fill the hero band
/// over the navy fill, so nothing shows through while it warms up. On any failure
/// it renders the navy fill only, and the hero's own image / discover-photo
/// fallback stays behind it.
class HeroBackgroundVideo extends StatefulWidget {
  ...
  /// https MP4/HLS stream. A YouTube link is deliberately excluded (see the class
  /// doc / D-761): it would need an Android WebView, which can't be clipped into
  /// the hero band. The hero only mounts the widget when this is true (otherwise
  /// it keeps the banner-image fallback).
...
class _HeroBackgroundVideoState extends State<HeroBackgroundVideo> {
  ...
  Future<void> _bindVideo(String url) async {
    ...
    try {
      ...
      // navy fill rather than spinning forever or crashing home. Dispose the
      // controller we created so a failed mount never leaks a native player.
      await controller.dispose();
      if (mounted) {
  ...
  /// Uses an [OverflowBox] (not a [FittedBox] transform) so the video Texture is
  /// sized natively rather than scaled under a transform.

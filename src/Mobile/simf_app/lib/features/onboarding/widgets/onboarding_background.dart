...
/// The onboarding step background (Figma 148:22): the active step's looping
/// muted video under a navy scrim; until the decoder is ready — or when it
/// never becomes ready (tests, an unsupported runtime, a codec the device
/// refuses) — the world-map poster shows instead.
///
/// Owner 2026-07-26 ("there is a video in the background — not exist / not
/// working") — two defects fixed here:
/// * the poster fallback was gated on `index == 0`, so a failed decode on step
///   2 / 3 left a **blank navy** screen with nothing behind the copy. The
///   poster now backs EVERY step, so the background is never empty.
/// * the scrim was the 90% photo overlay on the video too, which left roughly a
///   tenth of the footage visible — the video read as "not there". A playing

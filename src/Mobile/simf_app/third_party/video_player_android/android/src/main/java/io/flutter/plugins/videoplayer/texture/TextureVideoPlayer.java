...
public final class TextureVideoPlayer extends VideoPlayer implements SurfaceProducer.Callback {
  ...
  public static TextureVideoPlayer create(
    ...
    return new TextureVideoPlayer(
        ...
        options,
        () -> {
          // SIMF D-768 patch: some Huawei/HiSilicon AVC decoders
          // (OMX.hisi.video.decoder.avc) fail to initialise even though the format
          // is supported (format_supported=YES) — the hardware decoder is faulty.
          // setEnableDecoderFallback(true) makes ExoPlayer fall back to the next
          // decoder (the Android software AVC decoder, which plays 480p H.264 fine)
          // when the primary decoder throws at init. Also force SYNCHRONOUS
          // MediaCodec (async queueing trips the same chips at codec start()).
          // Both are harmless on devices whose hardware decoder works.

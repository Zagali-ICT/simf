...
/// |-------------|----------|---------|
/// | `Guest`     | 0 | Anonymous, or signed in but not yet approved. |
/// | `Visitor`   | 1 | Any registered attendee. |
/// | `Moderator` | 2 | محاور — runs a session's live Q&A desk. |
/// | `Staff`     | 3 | Venue gate staff (badge scan + walk-in sign-up). |
/// | `Exhibitor` | 4 | العارض — Visitor **plus** scan-QR + add-to-contacts. |
///
...
/// `guest < visitor < moderator < staff` comparisons that predate the set
/// model.
enum AppRole {
  guest(0, 'Guest'),
  visitor(1, 'Visitor'),
  ...
  /// (`"Visitor"`) or the wire int (`1`). Falls back to [AppRole.guest] when
  /// the value is null or unrecognised — safer than throwing on every unknown
  /// role, since a new backend role (Staff added later, etc.) would otherwise
  /// crash the app.
  ///
  /// **The live wire is the NAME, never the int** (`/app/users/me.appRole` and
  /// the `mobile_app_role` JWT claim both carry the string). The int branch is
  /// a defensive fallback only: this enum's integers intentionally differ from
  /// the backend `MobileAppRole` (here moderator=2/staff=3; backend Staff=2/

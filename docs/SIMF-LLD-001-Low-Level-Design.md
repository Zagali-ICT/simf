...
> every component is covered, with class/method/schema-level detail and sequence diagrams for
> the important flows, using representative examples rather than an exhaustive per-method
> catalogue. Companion: `SIMF-HLD-001` (High-Level Design). Authoritative contract docs:
> `SIMF-API-001`, `SIMF-DAT-001`, `SIMF-RPM-001`.
>
> **Verification note:** file paths, class names, routes, enum values and migration names below
> were read from source. Where the source itself marks something as not-yet-implemented or
...
This LLD describes **how** SIMF is built: project structure, the request pipeline, the classes
and interfaces that implement each cross-cutting concern, the endpoint pattern and feature
areas, the database schema, the two web front-ends, the Flutter app's layering, the shared
libraries, and the runtime flows that tie them together. It is detailed enough to implement
against and to review against.

### 1.2 How to read this document

Sections 2–6 cover the backend. Section 7 is the data model. Sections 8–11 cover the clients
and shared code. Section 12 collects sequence diagrams for the highest-value flows. Section 14
holds reference tables (enums, endpoints, migrations).
...
The dependency rule is strictly inward (Domain ← Application ← Infrastructure ← Api). This is
...
- **Dual OpenAPI documents** (D-247): an `app` document filtered to `/app/*` and a `cp`
  document filtered to `/admin/*`.
...
- **Swagger:** always on in Development; in Production gated behind Basic auth and an
  `AllowSwagger` flag, with a boot guard that refuses to start if enabled without credentials.
...
`ApiError` (`src/Shared/SIMF.Common/ApiError.cs`) carries `Code`, `Message`, `MessageArabic`,
and a list of `ApiErrorDetail` (`Field`, `Message`, `MessageArabic`). `ApiCallResult<T>` pairs
an HTTP status with the envelope so clients can branch on 401/403/etc. `DataValidationException`
(extends `ApiException`) is the domain-level validation error caught by the error middleware.
...
Validation uses FluentValidation `Validator<TRequest>` classes; failures are converted to HTTP
400 with `code = VALIDATION_FAILED` and bilingual field detail.

### 3.5 Pagination

- **Simple lists** (app GETs): `?page=&pageSize=&sort=&search=`; paging metadata returned in
  `meta`.
- **Admin grids** (POST): a `GridQuery` body (`Skip`, `Top`, `Search`, `Sort`, `SortDescending`,
  ...
  (`src/Shared/SIMF.Common/GridQuery.cs`). Default page size 20; grid cap 200; export/import
  caps higher.
...
- `CreateAccessToken(SimfUser, roles, permissions, MobileAppRole)` issues an HS256 JWT with
  claims: `sub`, `email`, `jti`, `display_name`, `security_stamp`, `account_state`,
  `user_type`, `mobile_app_role`, one `role` per role, and one `perm` per permission code (the
  Administrator collapses to a single `*`).
...
- **Refresh tokens** are stored hashed (`RefreshToken`, with `RotatedFromId` chain) and rotate
  on use; reuse of a rotated token is detected. The session has an absolute 24-hour cap
...
`PermissionCatalog` (`src/Shared/SIMF.Common/PermissionCatalog.cs`) is the single source of
truth:

- `Wildcard = "*"`, `ClaimType = "perm"`, `PolicyPrefix = "perm:"`.
- `PolicyFor(code)` → `"perm:" + code`; `IsPermissionPolicy`, `CodeFromPolicy` are the inverses.
- Codes are nested classes per page (`Visitors.View`, `Sessions.Edit`, `Bookings.Approve`,
  `Gates.Manage`, …); `PermissionCatalog.All` lists each with its baseline role(s) for
  idempotent seeding.
...
Named policies (in `AuthorizationPolicies`) include `RequireApprovedAccount`,
`AdministratorOnly`, and the gate policies (`GatesManage`, `GatesOperate`,
`GatesViewOwnReports`).

The same `PermissionCatalog` is reused by the Control Panel (see §8.3), so the API and the CP
enforce identical codes, and dedicated tests fail the build if a page/endpoint is ungated.
...
Two `SaveChangesInterceptor`s in `src/Backend/SIMF.Infrastructure/Auditing/`, registered in
order on the App context (the Identity context registers the row-audit interceptor):
...
2. **`RowAuditingSaveChangesInterceptor`** — captures every insert/update/delete into a
   `RowAudit` row (table name, entity type, operation, JSON primary key, old/new value JSON,
   affected columns, actor snapshot, correlation ID). Excludes append-only tables such as
   `GateScan`.

`IRequestContext` (`src/Backend/SIMF.Application/Abstractions/IRequestContext.cs`,
implementation `HttpRequestContext`) surfaces `SourceIp`, `UserAgent`, `CorrelationId`,
`ActorUserId`, `ActorDisplayName` without coupling services to `HttpContext`.

### 5.3 Operation log

`OperationLogEntry` (table `OperationLog`) is written explicitly by services for
security-relevant business events via `IAuditLog` / `AuditLog`
(`src/Backend/SIMF.Infrastructure/Auditing/AuditLog.cs`): `EventType`, `Outcome`, subject
(e-mail / user id / display name snapshot), actor (id / display name), `SourceIp`, `UserAgent`,
...
1. `ReserveSeatEndpoint` (`POST /app/sessions/{sessionId}/seats/reserve`) validates the request
   and resolves the caller from the JWT (`RequireApprovedAccount`).
...
- **`SimfAppDbContext`** (same folder) extends `DbContext`; receives `IPiiEncryptor`; history
  table `__EFMigrationsHistory_App`.
...
> `BaseAuditEntity` (`Id`, `CreatedAt/By`, `UpdatedAt/By`, `IsActive`, `DeletedAt`). Cross-DB
> user references are bare `Guid` "logical FKs".
...
(`IsForVisitor`, `MobileAppRole`), `UserInterest`, `Organisation`, `Country` (int key,
ISO-3166 code, `IsInvited`).

**Seats & venue:** `HallSeatLayout` (1:1 hall), `SeatReservation` (`Kind`, `Status`,
review cols, filtered unique index), `VenueMapNode` (`Kind`, X/Y, `HallId?`/`BoothId?`).

**Sessions content:** `SessionQuestion` (`Recipient`, `Phase`, `Status`, `AiFilterVerdict?`),
`SessionModerator`, `SessionComment` (`Status`, `LikeCount`, moderation cols),
`SessionCommentLike`.

**Exhibition:** `Booth` (officer cols, `ExhibitorId?`, `ContactId?`, map X/Y), `Exhibitor`,
`ExhibitorMembership`, `ExhibitorVisitorScan`, `Sponsor` (`Tier`, `Tagline(Arabic)`,
`ContactId?`).

**Media/news/archive:** `News`, `MediaItem` (`Kind`), `MediaPartner`, `ArchiveEdition`
(owns `ArchiveMediaItem`, `ArchiveSessionTitle`, `ArchivePastSpeaker`).

**Engagement:** `Rating` (overall + optional per-element stars), `ContentBlock`, `Banner`,
...
(owns `GateProfileTypeAllow`, `GateAssignment`), `GateScan` (append-only, bigint identity,
snapshot cols), `ScanIdempotency`, `SystemSetting`, `OrganizationProfile` (owns
`OrganizationAboutItem`, `OrganizationDetail`).
...
- `MobileAppRole`: None=0, Visitor=1, Staff=2, Moderator=3.
- `SessionStatus`: Scheduled=0, Held=1, Recorded=2, Published=3.
- `SeatReservationKind`: UserBooking=0, RandomAssignment=1, AdminReservedRow=2, OpenSeating=3.
- `BookingStatus`: Pending=0, Approved=1, Rejected=2.
- `SponsorTier`: Platinum=0, Gold=1, Silver=2, Bronze=3.
- `NotificationKind`: a by-name-persisted enum (e.g. BookingConfirmed=40, SessionReminder=41,
  BookingRejected=42, MeetingScheduled=43, MeetingCancelled=44).
- Plus venue/scan/meeting/networking/media/AI enums (Appendix 14.1).

### 7.5 Migration history

Each context has a single `InitialModel` migration plus a series of **additive** named
migrations (e.g. `D186_*`, `D199_EventModules`, `D217_*`, `D227..D230`, `D248_*`, `D260_*`,
`D463_*`, `SecA210_EncryptUserProfilePii`, `D495_AddOrganizationProfile`). The Identity schema is
frozen (D-110); App migrations land additively per controlled freeze-lift. Full list in
Appendix 14.3.
...
(`AppRenderMode.InteractiveServerNoPrerender`). Cookie authentication (`simf.cp.auth`,
8-hour sliding) with `SimfCookieRefreshHandler.OnValidatePrincipalAsync` rotating the access
token from the refresh token before expiry (D-121). The SignalR circuit allows large payloads
(QR/image transfer).
...
stores tokens in the encrypted cookie; the access token is held per-circuit in
`SimfAuthSession` and forwarded to the API by the typed client.
...
`CpNavigation` (`SIMF.ControlPanel/CpNavigation.cs`) defines ~12 nav groups (Overview, People,
Access Control, Programme, Scientific Committee, Exhibition, Engagement, Knowledge, Content,
Public Relations, Gates, Reference Data) over the `/admin/*` routes. Tests
(`CpNavigationPermissionTests`) fail the build if a page is ungated.

### 8.4 List-page and CRUD standard

Every list page uses `SimfDataGrid` (filter, select-all, row checkbox, quiet per-row icon
actions) with the shared CRUD framework (`CrudShell`, `CrudPageFrame`, `CrudDialogFrame`,
`CrudFormBase`, `CrudGridExcel`). Dialog-vs-full-page presentation is a per-page user preference
(`CpPreferences`, localStorage; D-353). Localization via `IStringLocalizer<Strings>` (resx
`Strings.resx`/`Strings.ar.resx`); `dir` is set from `CultureInfo.CurrentUICulture`.

---

## 9. Website (low-level design)

Blazor SSR for public content (`Home`, `Programme`, `Visit`) rendered statically for speed, with
**interactive islands** (`@rendermode AppRenderMode.InteractiveServerNoPrerender`) for the auth
and account flows: `Auth/SignIn`, `Auth/OtpVerify`, `Auth/ForgotPassword`, `Auth/ResetPassword`,
`Account/UserProfile`, `Account/Notifications`, `Account/PendingApproval`, `Account/Rejected`.
Cookie auth (`simf.web.auth`) with the same refresh handler; public reads use `SimfPublicClient`.
A smaller SignalR message-size cap reflects the lighter payloads. Auth is binary (signed-in vs
anonymous); the website has no per-page permission policies.
...
On 401 the client refreshes via `POST /app/auth/refresh` (with `skipAuthRefresh`) and replays
once. Concurrent 401s share **one in-flight refresh future** (single-flight, D-443).
Authenticated images use `getBytes()` (never bare `Image.network`, which cannot carry the bearer
token or the self-signed-TLS handling). A self-signed-TLS bypass is installed for the SIMF API
host only (development/PoC; no-op on web) — flagged as a security item to remove for production.

### 10.3 State, routing and theme

- **State:** Riverpod (Notifier + providers); auth state restored asynchronously on cold start
...
- **Routing:** go_router with `StatefulShellRoute.indexedStack` — a persistent 5-tab bottom nav
  ...
  (staff/moderator) redirect appropriately. The app privilege model is
  `AppRole`/`MobileAppRole` (Guest/Visitor/Staff/Moderator).
- **Theme:** navy-always (dark theme pinned), bilingual with automatic RTL for Arabic; a global
  app-bar invariant (notifications bell + language toggle + inert dark-mode indicator +
  hamburger) appears on every in-app page.

### 10.4 Screens

~34 feature folders under `lib/features/` covering: onboarding/splash/guest, auth (sign-in,
sign-up steps, OTP, badge sign-in/activation, biometric step-up), profile/registration, home,
sessions (+seat picker, my-seat, join hub), speakers, badge/QR, venue map, my-area (+identity
liveness), live broadcast, AI summary, news, gallery, archive, booths, sponsors, media partners,
comments, questions, meet-people, contacts (share/scan vCard), chatbot, exhibitor lead scan,
gate scan (staff), session moderation (moderator), my-meetings, feedback/rate, notifications,
accessibility, more, about, content/terms.
...
- **`SIMF.ApiClient`** — typed clients: `SimfAuthClient` (`api/v1/app/auth/`), `SimfAccountClient`
  ...
  Network faults are mapped to failed `ApiResult` envelopes (no exceptions to the caller). Base
  address resolved by `SimfApiBaseAddress` (HTTPS enforced outside Development; self-signed
  support flag).
- **`SIMF.Components`** — `Simf*` form/layout/control components, the CRUD framework, and the
  theme token CSS (`theme.tokens.css` as the single source of truth for colour/typography/
  spacing/radius/motion; light/dark/grey themes).
...
environment variables (double-underscore for nesting, e.g. `SIMF_Jwt__SigningKey`).
...
Boot guards refuse to start in production when required secrets are missing. **Note:** some
secrets exist in committed development config and must be rotated/purged before handover — see
the security remediation register; this is an owner/operations action, not a code item.
...
`NotificationKind` (by name) · `NotificationSeverity` (Info/Success/Warning/Error) ·
`MobileAppRole` · `Gender` · `SessionStatus` · `SessionType?` · `SessionSpeakerRole`
(Speaker/Host) · `SessionCommentStatus` · `QuestionStatus` · `QuestionPhase` ·
`SessionQuestionRecipient` · `SeatReservationKind` · `SeatSelectionMode` · `BookingStatus` ·
`VenueMapNodeKind` · `DirectionMode` · `ScanDirection` · `ScanOutcome` · `ScanSource` ·
`AttendanceMethod` · `HallPurpose` · `SponsorTier` · `ConnectionState` · `MeetingPartyKind` ·
`BusinessMeetingType` · `BusinessMeetingStatus` · `HallAllocationMode` · `MeetingRequestStatus` ·
`InvitationState` · `MediaKind` · `ArchiveMediaKind`. (Exact integer assignments are in
...
representative routes per feature area and the app/admin split convention
(`/api/v1/app/*` vs `/api/v1/admin/*`).

### 14.3 Migration history (per context)

- **Identity:** `InitialModel`, `AddSnapshotColumns`, `AddProfileTypeMobileAppRole`,
  `AddJobTitleToUserProfile`, `MoveProfilesToAppDb`, `AddDeviceKeys`,
  `D186_FoldOtherUsersIntoVisitor`, `SecA713_AddPasswordChangedAt`, `SecA720_AddPasswordHistory`,
  `SecA731_AddLastSuccessfulSignIn`.
- **App:** `InitialModel` → additive series including `AddSessions`, `AddOperationsToggles`,
  `MoveProfilesToAppDb`, `AddInvitations`, `AddSessionQuestions`, `AddCms`,
  `AddSeatReservations`, `AddAiModule`, `D186_*`, `D188_*`, `D199_EventModules`,
  `D202_*`, `D211_AddFaq`, `D217_*`, `D220..D226_*`, `D227..D233_*`, `D237/D240/D241_*`,
  `D248_AddBusinessMeetings`, `D260_AddContact`, `D269/D271..D278_*`, `D286_*`, `D357_AddAssets`,
  `D371/D373_*`, `D426/D429/D432/D439/D452/D456/D463/D472..D478/D485_*`,
  `SecA210_EncryptUserProfilePii`, `D495_AddOrganizationProfile`.

(See `src/Backend/SIMF.Infrastructure/Persistence/Migrations/{Identity,App}/` for the
canonical, timestamped list.)

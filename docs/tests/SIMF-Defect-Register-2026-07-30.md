...
| ~~`48-pre-existing-failures`~~ | build-ci | **CLOSED 2026-08-01 — measured, not inferred.** It sat here because confirming the count needed a full run, which a read-only survey could not do. A full run on `qa/programme-ws0` is **2,884 passed / 0 failed** (Api 2,227 · CP 423 · Web 117 · Application 63 · ApiClient 41 · E2E 8 · Domain 5), plus `flutter test` 1,312/1,312 and `flutter analyze` 0 errors / 0 warnings. The 48-failure allowance is spent; the note at `SIMF-Round1-Held-Items-Plan.md:160` now says so, because a stale allowance silently licenses 48 regressions. |

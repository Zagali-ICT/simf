...
> `SIMF-TST-001` strategy, a large per-page Gherkin E2E catalogue under
> [`e2e/`](e2e/), the FDS per-feature `T-01…T-NN` scenarios, and
...
>    columns (*Test / Comment / Time / Status / Developer comment*), that fuses
>    **every** existing per-page scenario **and** the new business flows into one
>    prioritised workbook.

### Catalogue size — where the number comes from

**This document deliberately does not state the size of the catalogue. Read it
from one of the two generated sources below, and do not copy it back into any
paragraph — including this one.**

The catalogue is appended to by many sessions working in parallel, so any figure
written into prose is stale almost immediately. That is not a theoretical
concern: this charter carried "164-file / 2,142-scenario" from 2026-07-11 to
2026-07-30, by which point the real catalogue was roughly **700 scenarios
larger** — a tester scoping a round from that sentence would have under-scoped it
by about a quarter. And the count moved again while this very section was being
written, as another session appended to an existing page.

| Source | What it gives you |
|---|---|
| The roll-up in [`e2e/README.md`](e2e/README.md) — `**Pages catalogued:**` / `**Total scenarios:**` | The canonical figure, and the one to quote. Pinned by `E2eCatalogueIntegrityTests.The_index_roll_up_matches_the_catalogue_it_describes`, which **fails the build** when it drifts from the directory it describes — so it cannot go stale without somebody noticing. |
| `python tools/testbook/build_testbook.py` | Re-derives the size on every run and emits it three ways: a `CATALOGUE SIZE` block on stdout, a dated provenance line on workbook sheet **00 · How to use**, and a **Catalogue size** block on sheet **01 · Summary**. It prints a MISMATCH warning (non-fatal, so the workbook still regenerates) when the README roll-up disagrees with the files. |

Both count the same thing on the same rule — an id in the first cell of a
Coverage-matrix row — so they are directly comparable. Re-derive before you plan
a round; a count is an input to scoping, never a fact about the system.
...
**Choice for this round (owner-approved): _Complete + prioritised_** — **every**
per-page scenario in the catalogue is present, Priority-tagged, with a
"Round-1 critical (P0/P1)" filter, alongside the fully-authored business flows.
The generated workbook states its own catalogue size on sheets **00 · How to
use** and **01 · Summary**; read it there rather than from this page (see
"Catalogue size" in the preamble).

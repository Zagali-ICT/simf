...
adb uninstall com.example.simf_app   # required if the installed build has a higher versionCode
adb install -r <apk>
```
...
| id | title | surface | steps | expected | data | blocked-by |
|----|-------|---------|-------|----------|------|------------|
| E2E-ONB-001 | Onboarding shows on first run only | app | `adb uninstall com.example.simf_app` → install → launch | Splash → **/onboarding**. Complete or skip it, kill and relaunch: the app goes straight to sign-in (`StorageKeys.onboardingCompleted`) | — | — |

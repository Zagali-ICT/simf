...
- **Speaker requests need the per-user eligibility flag (D-760, superseding the D-729 VIP-only rule).** Submitting a `SpeakerMeetingRequest` requires the requester's `UserProfile.AllowsSpeakerMeeting` to be true — admin-assigned per user, independent of the VVIP/VIP tier; the app hides the "Request a meeting" CTA when it is false and the server backstops with `403 Forbidden`. Scenario 002's requester therefore carries the flag. Scenario 004's `SPEAKER_MEETING_REQUESTS_NOT_ALLOWED` (a `409`) is checked *before* the eligibility gate, so a speaker who did not opt in is rejected regardless of the requester's eligibility.
...
Scenario: The central Q&A queue is gated by the Questions.* permissions
  ...
  And POST /admin/questions/list returns 403 for that principal
  ...
  And the grid is server-paged oldest-first, the page size capped at 200

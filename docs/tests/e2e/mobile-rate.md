...
| E2E-MOB040-027 | **The star control is operable by a screen reader (BUG-012)** — each of the 1–5 stars (overall bar and every per-element row) is a named button ("1 star" … "5 stars" / «نجمة واحدة» … «5 نجوم») reporting the current score as `selected`; before, they were bare tappable glyphs, so a screen-reader user could not submit a rating at all | a11y | P1 | authored ✓ (app `star_row_semantics_test` — named buttons, selected state, tap-by-label, Arabic labels) |
| E2E-MOB040-ELS-001 | Element inventory — every control the page wires is present, accessibly named, and correctly gated (no selection: selection-gated buttons present **and disabled**; one row selected: they enable). Asserted in **LTR and RTL**, expected-vs-actual against `tools/qa/predicted_inventory.py`. | element | P1 | _to author_ |
| E2E-MOB040-ELS-002 | Element health — no dead control, no broken image, and every same-origin link and asset returns < 400. Console reports zero errors and `scrollWidth == clientWidth` (no horizontal overflow). | element | P1 | _to author_ |
...
_Last reviewed:_ `2026-07-26` by `SIMF Team` (BUG-012 — the star controls are now
named, selectable semantics buttons; E2E-MOB040-027). Earlier: `2026-07-19` by Claude (owner attendance gate — rate only what you attended; blended HallAttendance + venue-gate GateScan; form `isEligible` flag); `2026-07-09` by Claude (D-713 — rate-on-hall-departure GAP-A + the watched-at header).

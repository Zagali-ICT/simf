...
| E2E-ORGP-013 | Hero video remove (D-768) — "Remove uploaded video" opens a "Remove hero video" SimfConfirm (D-799; nothing is sent until accepted), then clears `backgroundVideoUrl` and the stream `GET …/hero-video.mp4` then returns `404`; a separately-pasted external/YouTube link is left intact | happy | P1 | authored ✓ (`OrganizationHeroVideoTests` remove→404 + URL cleared) |

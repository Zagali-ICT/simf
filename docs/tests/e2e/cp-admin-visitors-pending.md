...
| E2E-VPN-007 | Bulk approve — select rows + "Approve selected" → confirm (D-799) → `Approved {n}. Skipped {m}.` | happy | P1 | _to author_ |
...
Scenario: Approve several pending visitors in one batch
  ...
  Then a SimfConfirm dialog opens titled "Approve selected accounts" naming the count (D-799)

...
| WEB-004 | Website landing — Partners strip | Logo marquee forced `direction:ltr` and scrolled **left** (`0→-50%`) on the RTL page → wrong-direction motion. | Owner report + live check 2026-06-08 | **FIXED + re-verified** (D-344) — reversed the keyframe to settle toward 0 (rightward / inline-end); live gate shows `translateX` increasing (−543→−524), console clean, no overflow. |
| WEB-005 | Website landing — Sessions (`الجلسات الرئيسية`) | Section renders a plain `.session-row` list; a richer tabbed-grid design was simplified (disabled CSS lines 821/871, "old session grid removed"). Owner: "not as in design". | Owner report 2026-06-08 | **OPEN — needs the design reference** (what the sessions section should look like) before restyling the polished page. |
| WEB-006 | Website landing — Archive (`الأرشيف`) | Auto-rotating year-tab timeline that swaps inline text; reads `/app/archive`. Owner: "on click must open real Archive, as in docs/mockup". | Owner report 2026-06-08 | **OPEN — needs the target** (what "open real archive" navigates to / opens per the mockup) before wiring the click. |
| WEB-007 | Website landing — Speakers (`أبرز المتحدثين`) | Cards are image-free **by design** (doc §7); reads `/app/speakers`, falls back to `SITE_DEFAULTS` demo when the backend has **no speakers seeded**. Owner: "not as in Backend, fill with demo data". | Owner report 2026-06-08 | **OPEN — needs a seed** (seed demo/real speakers into `/app/speakers` so the live section is populated, or enrich `SITE_DEFAULTS.speakers`). |

...
| C-1 | ~~One physical SQL Server database accessed through **two EF Core contexts** with separate migration histories.~~ **SUPERSEDED by D-157 (2026-05-29), reaffirmed D-246 (2026-06-02): two physically separated databases — `SIMF_Identity` (`SimfIdentityDbContext`) + `SIMF_App` (`SimfAppDbContext`), each with its own migration history, no cross-database relation/FK, no duplicated data.** Cross-DB user references are bare `Guid`s resolved on read. |

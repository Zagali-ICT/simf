...
| **Requester gate on a meeting request** | `SpeakerMeetingRequestService.AllowsSpeakerMeetingAsync` reads the per-user `UserProfile.AllowsSpeakerMeeting` (`…:959-965`), enforced in `SubmitAsync` (`…:97-103`); ineligible requester → 403 `Forbidden` | **Built.** D-760 replaced the former VIP-tier test (`IsVipAsync` over `ProfileType.AllowsVipMeetingSlots`), so eligibility no longer follows the audience tier. |

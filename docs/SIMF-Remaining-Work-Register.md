...
| # | Item | Evidence | How |
|---|------|----------|-----|
| 2.1 | ~~**Per-entity image pipeline**~~ **DONE.** Every `*RelativePath` / `*StoredFileName` string column is gone from the domain; each entity now points at `StoredFiles` with a real FK (`*FileId`) and a navigation. The D-895 carve-out that authorised this closes on evidence rather than a date, and its closing condition is met: `MediaPointerRatchetTests.KnownRemaining` is empty. | `tests/SIMF.Domain.Tests/MediaPointerRatchetTests.cs` (`KnownRemaining` empty, and it now also fails the build when an entity carries a `*FileId` alongside a fact the store already records); no `RelativePath` property remains under `src/Backend/SIMF.Domain`. | Done. Any further pointer conversion needs a new named lift - the carve-out is spent. |
| 2.2 | ~~**Missing CP per-page reference docs**~~ **DONE 2026-08-19.** The "~23" and its nine named examples were stale: eight of the nine already had docs. The real remainder was **12 live routes**, all now authored and indexed. **Read the index, not this row, for the count** - that is what made this item look twice its size. | `docs/pages/PAGE-INDEX.md` now has zero live rows with "—" in the Doc column (retired / removed / stub rows legitimately keep it). | Done. Authored from each page's razor + its existing E2E catalogue, then adversarially verified against source: 45 invented or overstated facts were caught and corrected before landing. |
...
| 2.4 | ~~**Full bUnit harness**~~ **DONE.** The row described the Sprint-1 state ("markup-source assertions only"), which has not been true for some time. | `bunit` 1.40.0 is a `PackageReference` in both `tests/SIMF.ControlPanel.Tests` and `tests/SIMF.Web.Tests`; 99 test files render components through it. | Done. |
| 2.5 | ~~**`AllowAnonymous` audit test.**~~ **DONE 2026-08-14.** `No_endpoint_outside_the_authentication_surface_is_anonymous` fails the build if any `/admin/` route is anonymous, or if any endpoint declares a policy **and** `AllowAnonymous`. The existing allow-list test was left as-is but renamed: it filters to `/auth/` before comparing, which is why it never covered this — worth knowing before trusting a guard by its name. | `tests/SIMF.Api.Tests/BusinessFlow13PermissionMatrixTests.cs`; both branches proven to fail by planting a violation, not merely observed green. | Done. **The second branch reads `EndpointDefinition.PreBuiltUserPolicies`, not `IAuthorizeData`** — FastEndpoints discards the policy metadata when `AllowAnonymous` is declared, so the obvious check passes silently against a genuinely open endpoint. Do not "simplify" it back. The full public-read surface is deliberately not pinned by name: those routes are legitimately anonymous and churn with content work. |
| 2.6 | ~~**D-110 freeze re-instatement (doc/ceremony half).**~~ **DONE 2026-08-14 — D-895.** All six lifts closed, including D-219's open-ended "further audit-surfaced additions". Sealed with a **test** (`SchemaFreezeTests`) rather than prose, and the baseline text corrected where three months of lifts had left it naming columns since renamed or never built. **§2.1's carve-out is now SPENT** — the image pipeline landed and `MediaPointerRatchetTests.KnownRemaining` is empty, which is the closing condition D-895 set. Any further pointer conversion needs a new named lift. | CLAUDE.md "### D-895 RE-INSTATEMENT"; `docs/decisions/DECISIONS_LOG.md` D-895. | Remaining: owner **sign-off** (the §3 part). The migration id no longer drifts — D-925 pinned it to `00000000000000_InitialCreate` on both contexts. **Updated 2026-09-07 (D-959):** the pinned id survives, but the regeneration workflow it served is retired — `Regenerate-Migration.ps1` refuses to run and a schema change is now its own `dotnet ef migrations add` migration, because a regenerated migration never reaches a database that already has one and the hand-run delta that was supposed to compensate was sometimes forgotten (D-944 broke production that way). |

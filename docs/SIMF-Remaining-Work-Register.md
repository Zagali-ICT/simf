...
| 2.6 | ~~**D-110 freeze re-instatement (doc/ceremony half).**~~ **DONE 2026-08-14 — D-895.** All six lifts closed, including D-219's open-ended "further audit-surfaced additions". Sealed with a **test** (`SchemaFreezeTests`) rather than prose, and the baseline text corrected where three months of lifts had left it naming columns since renamed or never built. **§2.1 keeps an explicit carve-out** — the image pipeline may still land. | CLAUDE.md "### D-895 RE-INSTATEMENT"; `docs/decisions/DECISIONS_LOG.md` D-895. | Remaining: owner **sign-off** (the §3 part), and re-pointing the migration ids in CLAUDE.md if §2.1 lands and re-mints them. |

...
| **Booking approval workflow** (FR-503 attendee seat reservations are confirmed immediately — no Control Panel approval — with the CP approval queue retained but dormant; FR-504 cancel before start; FR-502 no time-overlap) | API / CP / App | Seat *reservation* exists (`/sessions/{id}/seats/reserve`) and confirms immediately: attendee bookings auto-set `Status = Approved` on create, with no Control Panel approval step and no notification (the app shows an inline success message). The reservation is a **provisional hold** until the attendee checks in at the hall gate (staff QR scan), which confirms the seat; a pre-start sweep releases any hold not checked in shortly before the session starts. The old CP booking-approval surface (list-pending / approve / reject / bulk-approve, the CP **Bookings** page, the Bookings Approve/Reject permissions) is **retained but dormant** — nothing creates a Pending booking, so the queue is always empty. No no-overlap constraint. The **Bookings** nav item is a stub (`/m/bookings` → `ModulePlaceholder`). | Bookings = D-134 stub, never graduated |
...
| ~~**Live-stream Riyadh-region restriction** (FR-702)~~ **WITHDRAWN — there is no restriction to build.** | API / App | ~~Live screen is a provider-stub; no geo-restriction logic (and depends on the unbuilt geofence).~~ **Corrected 2026-07-31:** the owner re-scoped FR-702 from a geographic restriction to an **informational notice** — verbatim *"No restriction, this is only notification and be added to session."* A per-session bilingual free-text notice (`Session.LiveNotice` / `LiveNoticeArabic`, ≤512) is authored in the CP and shown **with** the stream on app screen #25 and the Website `/sessions/{id}`. No geo-detection, IP lookup or region gate exists or is planned, and the feed is served to every caller. This row's "not implemented" reading is therefore **wrong on both halves** — the notice is built, and the restriction is not scope. See SIMF-FDS-007 §5.1 (superseded wording kept and dated) and DECISIONS_LOG D-815. | D-199 → **D-815** |

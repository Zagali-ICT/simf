...
- [x] **#24** — Change-email flow (fixes CP new-account typos + app self-service) + re-verify + uniqueness. _(Identity is frozen)_ **✅ Built on main** — merged PR 156 (`862f6023`): app OTP self-service + backend `/app/auth/change-email` + CP re-verify + uniqueness, with tests + docs. **⚠ PARTIALLY REMOVED 2026-07-30 (G1, owner decision)** — the **self-service half** is gone (app screen + both endpoints deleted); the **CP half** stays and is now the only email-change path. See the [#24] detail row below.
...
- **Finding (by design):** `ProgrammeDay` has **no FK** from `Session`. The agenda buckets sessions under a day by matching the session's event-local (+03:00) start date to `ProgrammeDay.Date` (`ProgrammeSessionService`/`AdminSessionService` group by `Start.ToOffset(+03:00)`).
...
- **Finding (D-227):** today a visitor booking (`UserBooking` / `RandomAssignment` / `OpenSeating`) is created `BookingStatus.Pending` and held until an admin **Approves/Rejects** it; Pending holds auto-expire (`Expires`).
...
- **Current state (verified):** times are stored **UTC** — `Session.Start` / `End` (`DateTimeOffset`), audit stamping via `UtcNow`, all workers use `UtcNow` (`SessionReminderWorker`, `PendingBookingExpiryWorker`, rating-prompt workers, etc.). The agenda/booking layers convert UTC → `+03:00` (`EventOffset`) **only for display/grouping** (`ProgrammeSessionService`, `AdminSessionService`, `SeatReservationService`). So today: **storage = UTC, display = Saudi local**.
...
- **Surfaces to change:** `AuditStamping` interceptor, all workers (`SessionReminderWorker`, `PendingBookingExpiryWorker`, `ProgrammeRatingPromptWorker`, `SessionRatingPromptWorker`, `HallAttendanceCloseoutWorker`, `MeetingAwaitingSpeakerExpiryWorker`, `RegistrationGateAutoCloseWorker`), token/OTP expiry (NCA 5min/24h caps, D-443), `Session.Start/End` + every `*Utc` / `DateTimeOffset` field, reminder/rating dedup guards, EF default-value stamps.
...
- **Maps to existing model:** `SeatSelectionMode { AssignedSeat, OpenSeating }` (Hall + per-session override) = the two cases; `SeatReservation.Expires` = the auto-cancel guard (but the window must become **"3 min before Start"**, not "created + hold"); session attendance infra partly exists (`HallAttendance` / `HallAttendanceService` / `HallArrivalsConsole` / `GateScan`) — reuse for check-in/out.
...
- **Status (2026-07-22, history — do not delete):** ✅ Done — merged PR 156 (`862f6023`): app OTP self-service + `/app/auth/change-email` + CP re-verify + uniqueness, with tests + docs. Verified `origin/main` 55c232f1 (2026-07-23); see the Reconciliation banner.
- **Status (2026-07-30) — ⚠ SELF-SERVICE HALF REMOVED (G1, owner decision):** the owner ruled that a user may **not** change their own login email. Deleted: the Flutter `changeEmail` screen + its `More → Settings` row + repository + widget/golden tests, **both endpoints** (`POST /app/auth/change-email/send-otp` + `/confirm`), `EmailChangeService` / `IEmailChangeService`, the `EmailChange` contracts, the two request validators, the `EmailChange.*` audit events, the DI registration, the two rate-limit paths + the `newEmail` body peek, `ChangeEmailTests.cs`, `docs/pages/mobile/change-email.md` and `docs/tests/e2e/mobile-change-email.md` (namespace **MCE** retired).
- **Why the endpoints went too:** both were authenticated but declared **no** `PermissionCatalog` policy — only `RequireApprovedAccount` — so removing just the screen would have hidden the feature while leaving it callable by any approved bearer token.
- **What the original requirement asked for is still met:** the requirement was to fix **typos made when creating an account via the Control Panel**. That CP path is untouched — the account-edit form still changes an email, gated on `Visitors.Edit` / `Others.Edit` (both `BaselineRoles = AdminOnly`; **no new permission code was added**), and still sets `EmailConfirmed=false`, rolls the security stamp and revokes refresh tokens (`AdminAccountService.Update.cs`). It is now the **only** email-change path.
- **Regression guard:** `tests/SIMF.Api.Tests/ChangeEmailRemovedTests.cs` — both routes must return **404** (unmapped), authenticated and anonymously, so the endpoints cannot be reinstated silently.
- **Left in place on purpose:** `AccountCodePurpose.EmailChangeVerification` (=5) and `EmailTemplateType.EmailChangeVerification` (=7) / `EmailChangedNotice` (=8) are D-110 frozen and persisted **by name** — removing them is a data migration, not a code deletion — and their `EmailTemplateCatalog` entries stay dormant so admin-edited template rows are not orphaned.
...
- **As-built (2026-07-21; branch `feat/app-home-hero`, stacked on #42):**
  1. **Backend (code-only, NO migration/schema/freeze-lift):** `AssetCategory.Banner`(=8, append-only) wired into `AssetService.CategoryToService` + `OwnerIsActiveAsync` (public serve only while the banner is active AND within `[Start,End]`, matching `/app/banners`) + `AssetPermissionRegistry` (gated by `Banners.View/Edit`) + the Media-Library owner-name resolver. `FileService.Banner`/`FileOwnerEntityType.Banner`/its `PublicImage` policy already existed. `8a76a03f` (+ review fix `843c72c3`).

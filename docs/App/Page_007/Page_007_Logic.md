...
- **Profile image (C7, D-371):** required when `gender = male` — a camera capture must be attached, or the server must already store one (`hasIdImage` pre-fill); capture is **camera-only**; the image must pass the human-face check (on-device ML Kit + the server-side detector on the `id-image` endpoint — the server is the authority). **Not implemented as written:** the client's `noFaceDetectedError` string exists but no path renders it. The guided liveness loop skips a frame with no face (correct — most frames have none while the user positions themselves) and simply never advances, so a user who cannot get a face detected sees no message. Wiring it needs a no-face-for-N-seconds policy, which is an open product decision. The server-side detector on the `id-image` endpoint remains the authority.

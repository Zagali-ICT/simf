...
| Returns | `ApiResult<SignUpResult>` with HTTP **201** (generic, always) |

### Request
```jsonc
// SignUpRequest
{
  "email":    "string",   // required; valid email; server trims + lower-cases
  "password": "string"    // required; meets the password policy (SIMF-MOB-API-001)
}
```
> The **confirm-password** field is **client-only** (Page_005_Logic L-1) — it is
> validated on the device and is **not** part of the request body.
...
// ApiResult<SignUpResult>
{
  "succeeded": true,
  "data": {
    "message": "string"     // generic "check your email" message (AR/EN via headers)
  },
  "errors": []
}
```
...
### Error responses (`ApiResult<T>` with `succeeded: false`)

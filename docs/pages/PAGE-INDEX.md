...
| `myVisitors` (`GET /app/exhibitor/my-visitors`) — زواري, D-426 | ✅ Real — no Figma frame (functional page); **clean-code frozen (D-642)** — raw `RefreshIndicator` → shared `SimfPullToRefresh`; structural render-lock golden. Exhibitor's captured visitors (approved non-visitor only; 403 → forbidden surface) | Exhibitor (approved) | [mobile/my-visitors/](mobile/my-visitors/README.md) | — **(E2E catalogue pending — pre-existing D-426 gap)** |
| `scanVisitor` (`POST /app/exhibitor/scan` via `scanByBadge`) — مسح بطاقة زائر, D-426 | ✅ Real — no Figma frame (functional page); **clean-code frozen (D-643)** — already clean (85 lines, delegates to the shared `QrScanView`); structural render-lock golden (`enableCamera:false`). Exhibitor lead-capture scan → captures server-side → `myVisitors` | Exhibitor (approved) | [mobile/scan-visitor/](mobile/scan-visitor/README.md) | — **(E2E catalogue pending — pre-existing D-426 gap)** |

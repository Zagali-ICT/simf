...
| `/admin/visitors` | ✅ Real (2026-07-22: create-form regrouped into SimfFormSection cards + gated "Bulk add" batch-builder dialog, #10) | Administrator | [cp/admin-visitors.md](cp/admin-visitors.md) | [e2e/cp-admin-visitors.md](../tests/e2e/cp-admin-visitors.md) |
...
| #31 `mediaPartners` (`GET /app/media-partners` + logo `GET /app/assets/MediaPartnerLogo/{id}/image`) | ✅ Real — Figma `958:2246`; **clean-code frozen (D-630)**. Media-coverage hub partners tab + 2-col partner-card grid (real logos / initials fallback); shared `MediaCoverageTabs` (dedup done) | Guest+ | [mobile/media-partners/](mobile/media-partners/README.md) | [e2e/mobile-media-partners.md](../tests/e2e/mobile-media-partners.md) |
...
| #38 `accessibility` (no API) | ✅ Real — Figma `1116:16630`; **clean-code frozen (D-640)** — 4 widgets → `widgets/`, twin white label → `SimfTokens.labelWhiteMedium`, golden. Persisted + applied app-wide (D-327); screen-reader/captions wired (D-465) | Guest+ | [mobile/accessibility/](mobile/accessibility/README.md) | [e2e/mobile-accessibility.md](../tests/e2e/mobile-accessibility.md) |
...
| `changeEmail` (`POST /app/auth/change-email/send-otp` + `/confirm`) - Build #24 self-service login-email change | 🟢 Screen built (Build #24) · code emailed to the NEW address → confirm rolls the security stamp + revokes sessions → forces a fresh sign-in; reached from More → Settings (signed-in only); reuses the navy auth chrome (unbound) | Signed-in (approved) | [mobile/change-email.md](mobile/change-email.md) | [e2e/mobile-change-email.md](../tests/e2e/mobile-change-email.md) |

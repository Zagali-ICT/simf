...
| #116 `meetings` (`GET /app/my-requests` filtered to approved-upcoming meetings) — **اللقاءات الثنائية** (D-745, Figma `1408:9726`); **VIP-only** page split from the requests feed by the Home tile: approved+upcoming speaker/delegation meetings as photo/flag cards + طلب جديد create sheet (rich speaker picker) + السجل → history. Additive wire fields `speakerId`/`countryId` (no migration) | 🟢 Screen built (D-745) | **VIP** (approved) | [mobile/meetings.md](mobile/meetings.md) | [e2e/mobile-meetings.md](../tests/e2e/mobile-meetings.md) |

...
| `scanVisitor` (`POST /app/exhibitor/scan` via `scanByBadge`) — مسح بطاقة زائر, D-426 | ✅ Real — no Figma frame (functional page); **clean-code frozen (D-643)** — already clean (85 lines, delegates to the shared `QrScanView`); structural render-lock golden (`enableCamera:false`). Exhibitor lead-capture scan → captures server-side → `myVisitors`. **DEF-EXH-001/002/003:** the server authorises on `ProfileType.MobileAppRole == Exhibitor` (not "any non-visitor type"), the scanned subject must be an active audience account (ineligible → the same 404 as unknown), and a new capture raises `NotificationKind.ExhibitorLeadCaptured` to the visitor naming the exhibitor | Exhibitor (approved) | [mobile/scan-visitor/](mobile/scan-visitor/README.md) | [e2e/mobile-scan-visitor.md](../tests/e2e/mobile-scan-visitor.md) |

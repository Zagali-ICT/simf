...
| #117 `meetingConfirm` (`/meeting-confirm?requestId=…` → `POST /app/delegation-meeting-requests/{id}/confirm`) — **تأكيد الاجتماع**; bi-meeting rework: the delegation other-party confirm-on-tap screen, reached from a `MeetingRequested` notification deep link | 🟢 Screen built (bi-meeting rework) | Target-delegation member (`allowsDelegationMeeting`) | _(see e2e)_ | [e2e/mobile-meeting-confirm.md](../tests/e2e/mobile-meeting-confirm.md) |

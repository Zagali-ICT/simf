...
| 2026-07-27 | QA round (r2) | **Reopen now reaches the API.** The CP BFF proxy maps every upstream call explicitly (no catch-all), so the new row action needed `POST /account/api/admin/speaker-meeting-requests/{id}/reopen` in `AccountEndpoints` + `SimfAdminClient.ReopenSpeakerMeetingRequestAsync`; without them the button always fell into the generic load-failed toast. **The withdrawal email is now conditional:** it is sent only when the request was `AwaitingSpeaker` (the one status that actually emailed the speaker). A **Pending** cancel emails nobody — the speaker never saw that request, so the notice would have disclosed the requester's name + subject to an outsider and its "any confirmation link you received" line was false. E2E extended with E2E-SMR-030..031. |

_Last reviewed:_ 2026-07-27 by Claude (QA speaker-meeting round r2 — Reopen BFF proxy route, Pending-cancel email suppression).

...
| 2026-06-11 | D-357 | Speaker **photo** wired to the unified media-asset pipeline: `SimfImageUpload Category="SpeakerPhoto"` on the Add/Edit form + a `SimfImageThumb` of `/account/api/admin/assets/SpeakerPhoto/{id}/image` on Details/Deactivate. The same `SpeakerPhoto` asset drives the `/admin/speaker-presentations` profile-card wall and is preferred over the legacy `PhotoRelativePath` on the public website (`PublicSpeakerSummary.HasPhotoAsset` → `/content/assets/SpeakerPhoto/{id}/image`). E2E catalogue extended with E2E-SPK-023. |

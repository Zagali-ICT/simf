...
| **Backend endpoints** | `POST /account/api/admin/sessions/list`, `GET /admin/sessions/{id}`, `POST /admin/sessions`, `PUT /admin/sessions/{id}`, `DELETE /admin/sessions/{id}`, `PUT /admin/sessions/{id}/status`, `POST`/`DELETE /admin/sessions/{id}/recording`, `POST /admin/sessions/export`, `POST /admin/sessions/import` |

...
| **Last reviewed** | 2026-08-04 (D-843 — the PUT no longer drops the Hall binding) |
...
| 2026-08-04 | D-843 | **The Hall binding stopped being wiped by every edit.** `PUT /admin/gates/{id}` bound a hand-written route DTO that omitted `HallId` while `AdminGateService.UpdateAsync` assigns it unconditionally, so any save — renaming, toggling Active, changing operators — wrote `null` over the stored hall and silently demoted a hall-door gate to a perimeter gate. Since a perimeter gate never feeds `HallAttendance`, the visible symptom was session arrivals quietly not being recorded. Create was unaffected (it binds the contract type), which is why the suite stayed green. Fixed by converting the route DTO to the D-505 inheriting shape (`UpdateGateRequest : AdminUpdateGateRequest`) and passing the bound request straight through, so the drop is impossible rather than merely detectable. E2E extended with E2E-GAT-030. |
...
_Last reviewed:_ 2026-08-04 by Claude (D-843 — the PUT no longer drops the Hall binding). Prior: 2026-07-26 by Claude (BUG-018 — gate-operator model). Prior: 2026-06-11 by Claude (D-356 Phase 5 — Excel export + import + D-353 toggle).

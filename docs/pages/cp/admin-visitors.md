...
| 2026-07-27 | FR-PHN-002 | **Mobile numbers on the Edit form.** There was **no admin path to correct a phone number**: every CP surface showed `UserProfile.SaudiMobile` / `.InternationalMobile` read-only, and only the walk-in CREATE desk had an editable field — so a number typed wrong at registration left the account unreachable forever. `AdminUpdateVisitorRequest` / `AdminUpdateOtherRequest` gained **optional** `SaudiMobile` + `InternationalMobile`; `EditAccountForm` renders two `SimfTextField`s (`type=tel`, prefilled from the profile, with a format helper), and an empty field sends nothing so the stored number is untouched. Validation is the **same** `UpsertUserProfileRequestValidator.IsStandardSaudiMobile` / `IsStandardInternationalMobile` predicate the self-service upsert and the walk-in desk use — one source of truth — and a malformed number is a 400 raised before any write, so the whole edit rolls back. The value is stored **canonicalised** (DEF-PHN-003). Permission gate unchanged (`Visitors.Edit` / `Others.Edit`) — this extends an existing action, so no new permission. E2E-VIS-033. |
| 2026-08-16 | Mobile-number collapse | **One mobile column behind the two fields.** `SaudiMobile` and `InternationalMobile` were one attribute in two columns — a Saudi mobile is an international mobile with `+966` — so a row could hold two different numbers with nothing saying which to ring. The number now lives once in canonical E.164 on `UserProfile.MobileNumber` (PII-encrypted, `HasMaxLength(24)`), written by `ProfileMobileStorage.Sync`, which sets it and the two superseded columns together as exact complements (Saudi wins when both arrive, matching `VipRosterService`). **The Edit form and both request contracts are unchanged** — the two fields stay, and both wire keys stay, because the shipped app decodes them by name. Two behaviour changes to know: a Saudi local `05XXXXXXXX` is **stored folded** to `+9665XXXXXXXX` (storage path only — `MobileNumber.Canonicalize`, not `Normalize`, so acceptance is untouched), and "no change" is now per-**attribute**: supplying neither field leaves the row alone, supplying either one **replaces** the stored number rather than sitting beside it. E2E-VIS-033. |

---

_Last reviewed:_ 2026-08-16 by Claude (the mobile-number collapse — one canonical `UserProfile.MobileNumber` behind the two unchanged Edit fields). Earlier: 2026-07-27 by Claude (FR-PHN-002 — mobile numbers on the Edit account form). Earlier: 2026-07-26 by Claude (B22 — nationality on the Edit account form). Earlier: 2026-07-14 by Claude (D-568 — profile-photo thumbnail in the list). Earlier: 2026-07-09 by SIMF Team (D-728 — change-account-type action); 2026-06-10 (D-356 Phase 5 — D-353 Page↔Popup toggle note; Excel N/A for this account page).

...
| **Backed by** | `dbo.SpeakerPresentations` table (migration `D228_AddSpeakerPresentations`, 2026-06-02). Bytes stored **out-of-row** in the one `StoredFiles` table; the row holds the metadata plus `StoredFileId`, a real foreign key (D-888). |
